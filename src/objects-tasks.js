...
function mergeObjects(objects) {
  ...
  const ent1 = Object.entries(objects[0]);
  const ent2 = Object.entries(objects[1]);

  ent1.map((value) => {
    ent2.map((val) => {
      if (value[0] === val[0]) {
        result[value[0]] = value[1] + val[1];
      }
      return val;
    });
    return value;
...
function makeWord(lettersObject) {
  const result = [];
  const arr = Object.entries(lettersObject).flat(Infinity);
  let currLet = '';
  let posLet = 0;
  for (let i = 0; i < arr.length; i += 1) {
    ...
    if (typeof arr[i] === 'number' && typeof arr[i + 1] === 'number') {
      let pos = i;
      if (typeof arr[pos - 1] !== 'string') {
        pos -= 1;
      }
      currLet = arr[pos - 1];
      posLet = arr[i + 1];
    }
    result[posLet] = currLet;
  }
  return result.join('');
}
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    if (queue[i + 1]) {
      change += queue[i];
      if (queue[i + 1] - change > 25) {
        return false;
      }
    } else if (queue[queue.length - 1] - change > 25) {
      return false;
    }
  }
  return true;
